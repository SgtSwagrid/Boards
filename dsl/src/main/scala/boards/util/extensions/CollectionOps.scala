...
object CollectionOps:
  ...
  extension [X] (seq: Seq[X])
    ...
    def takeTo(f: X => Boolean): Seq[X] =
      seq.takeUntil(f) ++ seq.dropUntil(f).headOption
  ...
  extension [X] (seq: Seq[Seq[X]])
    
    def cartesianProduct: LazyList[List[X]] =
      LazyList.from(seq).map(LazyList.from).foldRight(LazyList(List.empty[X])): (word, sentence) =>
        for word <- word; sentence <- sentence yield word :: sentence
      
  extension [X] (itr: Iterator[X])
    
    def toLazyList: LazyList[X] =
      
      def fromItr(itr: Iterator[X]): LazyList[X] =
        if itr.hasNext then LazyList.cons(itr.next(), fromItr(itr))
        else LazyList.empty
